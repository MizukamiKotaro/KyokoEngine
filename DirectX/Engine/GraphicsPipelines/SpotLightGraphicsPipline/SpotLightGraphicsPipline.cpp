...
SpotLightGraphicsPipline* SpotLightGraphicsPipline::GetInstance()
{
	static SpotLightGraphicsPipline instance;
	return &instance;
}

void SpotLightGraphicsPipline::Initialize()
{
...
void SpotLightGraphicsPipline::PreDraw()
{
...
void SpotLightGraphicsPipline::SetBlendMode(uint32_t blendMode)
{
...
void SpotLightGraphicsPipline::InitializeDXC()
{
	//DXCの初期化
...
void SpotLightGraphicsPipline::InitializePSO()
{
	//DescriptorRange
	...
	//Shaderをコンパイルする
	vertexShaderBlob_ = CompileShader(L"Resources/Shaders/SpotLightShader/SpotLight.VS.hlsl", L"vs_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = CompileShader(L"Resources/Shaders/SpotLightShader/SpotLight.PS.hlsl", L"ps_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
...
IDxcBlob* SpotLightGraphicsPipline::CompileShader(const std::wstring& filePath, const wchar_t* profile, IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler)
{
	// 1. hlslファイルを読む
