...
namespace ImGuiCommon {
    ...
    void DopeSheet() {
        ...
        float currentY = 20.0f; // 開始位置をタイムラインラベルの下に設定

        for (TimerTrack& track : tracks) {
            DrawTrack(drawList, track, cursorScreenPos, contentRegion, timelineScale, scrollOffset, currentY, 0, rightClickPos, clickedTrackIndex, trackIndex);
            trackIndex++;
        }
