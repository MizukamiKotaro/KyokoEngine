...
PixelShaderOutput main(VertexShaderOutput input) {
	...
	float32_t4 textureColor = gTexture.Sample(gSampler, nput.texcoord);
	...
	if(n != 0){
		...
		for(int i = 0; i < gWaterOutline.outlinePix; i++){
			textureColor = gTexture.Sample(gSampler, nput.texcoord + float32_t2((-i - 1) * x,0));
			n = n * textureColor.a;

			textureColor = gTexture.Sample(gSampler, nput.texcoord + float32_t2((i + 1) * x,0));
			n = n * textureColor.a;

			textureColor = gTexture.Sample(gSampler, nput.texcoord + float32_t2(0,(-i - 1) * y));
			n = n * textureColor.a;
