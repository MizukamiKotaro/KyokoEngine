...
void main(uint32_t3 DTid : SV_DispatchThreadID)
{
	...
	if(vertexIndex < gSkinningInformation.numVertices) {
		...
		skinned.ambientColor = input.ambientColor;
		skinned.specularColor = input.specularColor;
		skinned.shininess = input.shininess;
		skinned.sphereTextureNum = input.sphereTextureNum;
		skinned.toonTextureNum = input.toonTextureNum;
